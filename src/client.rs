...
impl CopyMangaClient {
    ...
    /// 带重试的 HTTP 请求
    async fn send_with_retry(&self, req: reqwest::RequestBuilder, max_retries: u32) -> Result<reqwest::Response> {
        let mut last_err = None;
        for attempt in 0..=max_retries {
            if attempt > 0 {
                tokio::time::sleep(Duration::from_secs(1)).await;
                warn!("重试请求 (第{}次)", attempt);
            }
            match req.try_clone().unwrap().send().await {
                Ok(resp) => {
                    if resp.status().is_server_error() && attempt < max_retries {
                        last_err = Some(eyre!("服务器错误: {}", resp.status()));
                        continue;
                    }
                    return Ok(resp);
                }
        ...
        Err(last_err.unwrap_or_else(|| eyre!("请求失败")))
    }
    ...
    pub async fn login(&self, username: &str, password: &str) -> Result<String> {
        ...
        let token = data["token"].as_str().ok_or_else(|| eyre!("登录返回没有 token"))?.to_string();
        Ok(token)
    }
    ...
    pub async fn get_comic(&self, path_word: &str) -> Result<GetComicRespData> {
        let resp = self.send_with_retry(
            self.api_client
                .get(format!("https://{}/api/v3/comic2/{path_word}", self.api_domain))
    ...
    pub async fn get_group_chapters(&self, comic_path_word: &str, group_path_word: &str) -> Result<Vec<ChapterItem>> {
        let mut all = vec![];
        let limit = 100;
        let first = self.get_chapters(comic_path_word, group_path_word, limit, 0).await?;
        all.extend(first.0.list);
        let total_pages = first.0.total / limit + 1;
        if total_pages <= 1 { return Ok(all); }
        for page in 2..=total_pages {
            let offset = (page - 1) * limit;
            all.extend(self.get_chapters(comic_path_word, group_path_word, limit, offset).await?.0.list);
        }
        all.sort_by(|a, b| a.ordered.cmp(&b.ordered));
        Ok(all)
    }

    async fn get_chapters(&self, comic_path_word: &str, group_path_word: &str, limit: i64, offset: i64) -> Result<GetChaptersRespData> {
        let resp = self.send_with_retry(
            self.api_client
                .get(format!("https://{}/api/v3/comic/{comic_path_word}/group/{group_path_word}/chapters", self.api_domain))
    ...
    pub async fn get_chapter_images(&self, comic_path_word: &str, chapter_uuid: &str) -> Result<Vec<(String, i64)>> {
        let authorization = if self.token.is_empty() { String::new() } else { format!("Token {}", self.token) };
        let mut req = self.api_client
            .get(format!("https://{}/api/v3/comic/{comic_path_word}/chapter2/{chapter_uuid}", self.api_domain))
            .query(&json!({"platform": 1}));
        if !authorization.is_empty() {
            req = req.header("authorization", &authorization);
        }
        let resp = self.send_with_retry(req, 2).await?.json::<CopyResp>().await?;
        if resp.code != 200 {
            return Err(eyre!("获取章节图片失败: {}", resp.message));
        }
        let data: GetChapterRespData = serde_json::from_value(resp.results)?;
        let urls: Vec<(String, i64)> = data.chapter.contents.into_iter().enumerate()
            .map(|(i, c)| (c.url.replace(".c800x.", ".c1500x."), data.chapter.words.get(i).copied().unwrap_or(i as i64)))
            .collect();
        Ok(urls)
    }
